...
import argparse
from basic_utils.utils_torch_intent import compute_values, get_data, compute_values_eval
from experiment_oproto_intent import RunExperiment
from workspace.workspace_intent import workspace
from models.model_oproto_intent import *
        ...
        for b in range(len(self.data)):
            ...
            for target_set, target_set_file in zip(qcurr_dev_workspace.target_sets, qcurr_dev_workspace.target_sets_files):                  
                ...
                if str(target_set_file).find('test') != -1:
                    ...
                    #print("len x test set:", len(input_target_sent_test))
...
def eval_model(params, model, experiment, optimizer, epoch):
    ...
    for c in range(len(test_set)):
        ...
        save_pickle(RSL_PATH, 'metaval_oproto_intent_%s_%s.pkl'%(c,epoch), (v_macro_avg_eer, v_macro_avg_acc_ideal, v_macro_avg_acc))
        save_pickle(RSL_PATH, 'vprobs_gts_oproto_intent_%s_%s.pkl'%(c,epoch), (v_probs, v_gts))
        ...
        save_pickle(RSL_PATH, 'metatest_oproto_intent_%s_%s.pkl'%(c,epoch), (t_macro_avg_eer, t_macro_avg_acc_ideal, t_macro_avg_acc))
        save_pickle(RSL_PATH, 'tprobs_gts_oproto_intent_%s_%s.pkl'%(c,epoch), (t_probs, t_gts))
    ...
    save_pickle(RSL_PATH, 'metaval_oproto_intent_all_%s.pkl'%epoch, (v_eer_, v_acc_ideal_, v_acc_))
    save_pickle(RSL_PATH, 'vprobs_gts_oproto_intent_all_%s.pkl'%epoch, (v_probs_, v_gts_))
    ...
    save_pickle(RSL_PATH, 'metatest_oproto_intent_all_%s.pkl'%epoch, (t_eer_, t_acc_ideal_, t_acc_))
    save_pickle(RSL_PATH, 'tprobs_gts_oproto_intent_all_%s.pkl'%epoch, (t_probs_, t_gts_))

       
if __name__ == '__main__':


    parser = argparse.ArgumentParser(description="Evaluate O-Proto on intent classification.")
