...
import argparse
from basic_utils.utils_torch_cls import compute_values, get_data, compute_values_eval
...
class AmazonLoader(Dataset):
    ...
    def get_train_batch(self, train_workspace, all_ood_workspaces):
        ...
        #print("sentence_size_per_intent:", sentence_size_per_intent)
        cls_ = np.arange(2)
...
def train_model(params, model, experiment, optimizer):
    ...
    save_pickle(DATA_PATH, 'tr_dev_te_sentiment.pkl', (train_data, dev_data, test_data))
        ...
        for id_tr in b:
            ...
            if len(train_loss_all) > 1:
                ...
                if (avg_loss/it) <= min_train_loss:
                    ...
                    torch.save(state, open(os.path.join(RSL_PATH, 'imax_sentiment_k100.best.pth'), 'wb'))
...
def eval_model(params, model, experiment, optimizer, epoch):
    ...
    for c in range(len(test_set)):
        ...
        save_pickle(RSL_PATH, 'metaval_imax_cls_%s_%s.pkl'%(c,epoch), (v_macro_avg_eer, v_macro_avg_acc_ideal, v_macro_avg_acc))
        save_pickle(RSL_PATH, 'vprobs_gts_imax_cls_%s_%s.pkl'%(c,epoch), (v_probs, v_gts))
        ...
        save_pickle(RSL_PATH, 'metatest_imax_cls_%s_%s.pkl'%(c,epoch), (t_macro_avg_eer, t_macro_avg_acc_ideal, t_macro_avg_acc))
        save_pickle(RSL_PATH, 'tprobs_gts_imax_cls_%s_%s.pkl'%(c,epoch), (t_probs, t_gts))
    ...
    save_pickle(RSL_PATH, 'metaval_imax_cls_all_%s.pkl'%epoch, (v_eer_, v_acc_ideal_, v_acc_))
    save_pickle(RSL_PATH, 'vprobs_gts_imax_cls_all_%s.pkl'%epoch, (v_probs_, v_gts_))
    ...
    save_pickle(RSL_PATH, 'metatest_imax_cls_all_%s.pkl'%epoch, (t_eer_, t_acc_ideal_, t_acc_))
    save_pickle(RSL_PATH, 'tprobs_gts_imax_cls_all_%s.pkl'%epoch, (t_probs_, t_gts_))
