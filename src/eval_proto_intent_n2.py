...
from models.model_proto_intent import *
...
if __name__ == '__main__':
    ...
    voc, w2v = load_w2v()
    word2idx, idx2word = read_pickle('/content/protoinfomax/data/', 'dict_idx2word_intent_kw.pkl')
    ...
    for ep in [0,10,20,30,40,50,60,'best']:
        ...
        if ep =='best':
            checkpoint = torch.load(os.path.join('/content/protoinfomax/results/', 'proto_intent_k100.best.pth'), map_location=lambda storage, loc: storage)
        else:
            checkpoint = torch.load(os.path.join('/content/protoinfomax/results/', 'proto_intent_k100_%s.pth'%ep), map_location=lambda storage, loc: storage)
