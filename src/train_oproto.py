...
def train_model(params, model, experiment, optimizer):
    ...
    train_data, _, _ = read_pickle(DATA_PATH, 'tr_dev_te_sentiment.pkl')
    ...
    for e in range(0, total_epoches):
        ...
        # sample 10 training subsets-id from 100 batch examples:
        b = np.random.choice(len(train_set), 10, replace=False)
        for id_tr in b:
            ...
            if len(train_loss_all) > 1:
                ...
                if (avg_loss/it) <= min_train_loss:
                    ...
                    torch.save(state, open(os.path.join(RSL_PATH, 'oproto_sentiment_k100.best.pth'), 'wb'))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Training OProto on sentiment classification.")
